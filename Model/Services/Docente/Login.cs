            ...
            try
            {
                using (var ctx = new CAS_DataEntities())
                {
                    ...
                    exite = (from usuario in ctx.CAS_USUARIO
                             ...
                             where usuario.COD_USUARIO.Trim() == login.COD_USUARIO.Trim() && usuario.CLAVE_USUARIO.Trim() == login.CLAVE_USUARIO.Trim() && rol.ID_ROL == 7
                             select new
                             {
            ...
            try
            {
                using (var ctx = new CAS_DataEntities())
                {
                    _loginCredentials = (from usuario in ctx.CAS_USUARIO.Where(x => x.COD_USUARIO == login.COD_USUARIO && x.CLAVE_USUARIO == login.CLAVE_USUARIO)
                                         ...
                                         join docente in ctx.CAS_DOCENTE on usuario.ID_USUARIO equals docente.ID_DOCENTE
                                         select new Login
                                         {
                                             ID_USUARIO = usuario.ID_USUARIO,
                                             COD_USUARIO = usuario.COD_USUARIO,

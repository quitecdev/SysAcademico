...
namespace Model.Services.Admin
{
    public class InscripcionDetallleCarrera
    {
        ...
        public void EliminarDetalleCarrera(int _ID_INSCRIP_DETALLE_CARRERA)
        {
            CAS_INSCRIPCION_DETALLE_CARRERA insCarrera = new CAS_INSCRIPCION_DETALLE_CARRERA();
            CAS_ESTUDIANTE_DETALLE estCarrera = new CAS_ESTUDIANTE_DETALLE();
            try
            {
                using (var ctx = new CAS_DataEntities())
                {
                    insCarrera = ctx.CAS_INSCRIPCION_DETALLE_CARRERA.Where(x => x.ID_INSCRIP_DETALLE_CARRERA == _ID_INSCRIP_DETALLE_CARRERA).FirstOrDefault();

                    if (insCarrera != null)
                    {
                        estCarrera = ctx.CAS_ESTUDIANTE_DETALLE.Where(x =>
                                        x.ID_ESTUDIANTE == insCarrera.ID_INSCRIP &&
                                        x.ID_CARRERA == insCarrera.ID_CARRERA &&
                                        x.ID_PERIODO == insCarrera.ID_PERIODO &&
                                        x.ID_SEDE == insCarrera.ID_SEDE
                                    ).FirstOrDefault();
                        if (estCarrera != null)
                            ctx.Entry(estCarrera).State = EntityState.Deleted;
                    }
                    ctx.Entry(insCarrera).State = EntityState.Deleted;
                    ctx.SaveChanges();
                }
            }
            catch (Exception ex)

...
namespace Model.Services.Admin
{
    public class Docente
    {
        ...
        public List<Docente> cmbDocente()
        {
            List<Docente> _docente = new List<Docente>();
            try
            {
                using (var ctx = new CAS_DataEntities())
                {
                    _docente = (from docente in ctx.CAS_DOCENTE
                                ...
                                {
                                    ID_DOCENTE = x.ID_DOCENTE,
                                    APELLIDO_PATERNO_DOCENTE = x.APELLIDO_PATERNO_DOCENTE+' '+ x.APELLIDO_MATERNO_DOCENTE+' '+ x.PRIMER_NOMBRE_DOCENTE+' '+ x.SEGUNDO_NOMBRE_DOCENTE,
                                }).OrderBy(x=>x.APELLIDO_PATERNO_DOCENTE).ToList();
